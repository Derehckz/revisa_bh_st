...
def _guardar_excel_revision(ruta_mes, df, estadisticas):
    ...
    def _writer(tmp_path: str) -> None:
        ...
        if OPENPYXL_AVAILABLE:
            _aplicar_formato_condicional_excel(tmp_path, ruta_mes)

    utils.atomic_excel_write(salida_excel, _writer)
...
def main():
    ...
    if utils.is_non_interactive():
        if not args.year:
            args.year = os.environ.get("BH_YEAR", "").strip() or None
        if not args.month:
            args.month = os.environ.get("BH_MONTH", "").strip() or None
        if not args.year or not args.month:
            utils.print_error(
                "Modo no interactivo: indique --year y --month (carpetas del período) o defina BH_YEAR/BH_MONTH."
            )
            return
    ...
    if año not in años:
        utils.print_error(f"El año '{año}' no existe en {RAIZ}")
        return
    ...
    if mes not in meses:
        utils.print_error(f"El mes '{mes}' no existe en {ruta_año}")
        return
    ...
    continuar = utils.mostrar_contexto_ejecucion(
        ...
        [
            ...
            ("Procesos", str(max(1, args.processes))),
        ],
        preview_items=["Se revisarán carpetas IP/CFT y se generará Excel de control."],
        confirm_message="¿Continuar con la revisión de carpetas? (s/n)",
    )
    if not continuar:
