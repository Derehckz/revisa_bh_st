...
export function StageHeaderBanner({ stage }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/50 px-3 py-2 text-sm">
      <span className="text-muted-foreground">Estado del paso:</span>
      <Badge tone={STATUS_TONE[stage.ui_status]}>{STATUS_LABEL[stage.ui_status]}</Badge>
      {last && (
        <span className="text-xs text-muted-foreground">
          Última ejecución ({last.source === "filesystem" ? "consola" : "web"}):{" "}
          <strong title={last.label ?? last.id}>{last.label ?? last.id}</strong>
      ...
      {!stage.prerequisites.ok && (
        <span className="text-xs text-amber-800">{stage.prerequisites.message}</span>
      )}
    </div>
  );
}
